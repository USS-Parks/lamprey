...
|---|---|---|
| **NSIS installer** (Windows) | `Lamprey-x64.exe` | [Download .exe](https://github.com/USS-Parks/lamprey/releases/download/v0.15.1/Lamprey-x64.exe) |
| **Portable ZIP** (Windows) | `Lamprey-x64.zip` | [Download .zip](https://github.com/USS-Parks/lamprey/releases/download/v0.15.1/Lamprey-x64.zip) |

Or browse all releases → <https://github.com/USS-Parks/lamprey/releases>

**New in v0.15.1 — Loop Phase gap-closure.** Closes the four honest gaps the Loop Phase shipped with. **Settings → Loops tab** — the master toggle plus all five ceilings (max iterations, max wall-clock, token budget, max concurrent, runaway floor) now have a real UI; no more editing `settings.json`. **Status-line slot** — a running loop shows "N loop · iter X" at the bottom of the window (hidden when none are running, so default installs see nothing). **Context-aware token budget** — the per-iteration estimate now counts the full message stack sent to the model (system prompt + history + prompt) plus the reply, instead of the iteration prompt alone, so the soft token cap actually tracks real spend (iteration + wall-clock remain the hard caps). **Real DB integration test** — the Electron-built better-sqlite3 can't load under the test runner's Node ABI, so the loop DB suites used to skip; a new test runs the *exact* migration-v17 DDL and the loop query shapes against Node's built-in `node:sqlite` (no native addon), giving 9 always-running assertions for schema + query correctness instead of relying on the manual playbook. Verify gate: tsc node + web clean, full vitest green, build OK, `verify:proof` exit 0. See [PLANNING/LP_SMOKE_PLAYBOOK.md](PLANNING/LP_SMOKE_PLAYBOOK.md).

**Previously in v0.15.0 — Loop Phase.** A first-class **Loop** primitive — and a *deliberate, documented extension past the Opus 4.5 era-lock* (the first feature Lamprey ships that the era product didn't have; loops are **off by default**, so the era-faithful default experience is unchanged). Twelve prompts (LP-0 through LP-11) spanning the full autonomy gradient: **interval** (`/loop 5m <task>` — fixed cadence), **self-paced** (`/loop <task>` — the model paces itself), and **autonomous "work the backlog"** (`/loop --auto <mission>` — the loop drains a dedicated queue and the model grows it with `loop_enqueue`, records outcomes with `loop_complete_task`, and self-terminates with `loop_control(mission_complete)`). The headline fix: a pre-existing half-built `schedule_wakeup` scaffold injected a wake-up message but **never ran a turn** (it just reloaded the UI) — LP-1 factors a **headless main-process turn runner** out of `chat:send` so a fired wake-up (and every loop iteration) runs a real turn with the window closed or another conversation focused. Every loop is bounded by **hard ceilings** — max iterations, wall-clock, token budget, a per-iteration stall watchdog, and a runaway interval floor the model cannot schedule under — with three stop authorities (user pause/stop, model self-terminate, guard trip) and a progress ledger that keeps settled work from being redone. A new **Loops** right-panel pill shows each loop's live status / iteration / budget / next-fire countdown and manages its backlog (add / remove / reorder). Settings ship `loopsEnabled: false` + five bounded defaults, parity-locked. The autonomous machinery exists but stays opt-in, never default — consistent with the era lock. Verify gate: tsc node + web clean, full vitest 2240 passed / 130 skipped / 0 failed, build OK, `verify:proof` exit 0. Honest gaps: no Settings UI tab yet (edit `settings.json`), no status-line slot for loop entities (the panel is the surface), token budget is approximate (iteration + wall-clock are the hard caps), and the DB-backed loop tests skip under the better-sqlite3 binding mismatch so [PLANNING/LP_SMOKE_PLAYBOOK.md](PLANNING/LP_SMOKE_PLAYBOOK.md) is the live integration gate. See [PLANNING/LAMPREY_LOOP_PLAN.md](PLANNING/LAMPREY_LOOP_PLAN.md) and [PLANNING/LP_BASELINE.md](PLANNING/LP_BASELINE.md).

**Previously in v0.14.0 — Unburdening Phase.** The deletion release. Thirteen prompts (UB-0 through UB-12) that strip out — not gate, remove — every subsystem the Claude Code / Opus 4.5 era product never had: the **multi-agent Planner→Coder→Reviewer pipeline** and its dispatch, the **auto-routing heuristic** and its telemetry, the **mechanical proof gate** (rigor scoping, change-contract synthesis, receipts scan, "Untrusted completion" banners), the **final-response composer** (the reply you read is now the model's reply, byte-for-byte), and all of their chrome — stage chips, pipeline traces, stage budgets, mode toggles, the Agents settings tab. **Net −7,400 lines.** Historical pipeline messages keep a single muted "Pipeline (legacy)" chip; the DB schema and store layers stay so nothing you wrote is lost; git history at the v0.13.0 tag holds the last full-machinery build. The system prompt contract drops to ~3.1 KB and the byte guards tightened accordingly. What remains is the product as declared: one model per turn with its full tool catalog, read–edit–verify, skills, MCP, research, RAG, the panels aesthetic, and the model-callable `multi_agent_run` tool for when the *model* chooses to fan out. Verify gate: lint clean, tsc node + web clean, full vitest green, build OK, `verify:proof` exit 0. See [PLANNING/LAMPREY_UNBURDENING_PLAN.md](PLANNING/LAMPREY_UNBURDENING_PLAN.md) and [PLANNING/UB_BASELINE.md](PLANNING/UB_BASELINE.md).
