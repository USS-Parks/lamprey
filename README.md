...
|---|---|---|
| **NSIS installer** (Windows) | `Lamprey-x64.exe` | [Download .exe](https://github.com/USS-Parks/lamprey/releases/download/v0.9.2/Lamprey-x64.exe) |
| **Portable ZIP** (Windows) | `Lamprey-x64.zip` | [Download .zip](https://github.com/USS-Parks/lamprey/releases/download/v0.9.2/Lamprey-x64.zip) |
...
1. **Download** the [v0.9.2 installer](https://github.com/USS-Parks/lamprey/releases/download/v0.9.2/Lamprey-x64.exe) and run it.
...
- ✅ **Schema Bootstrap Hotfix** (v0.9.2) — P0 fix for a v0.9.1 regression where every chat send failed with `table messages has no column named proof_status`. Root cause was actually upstream of WC-4's migration v16: the `conversation_rag_attachments` DDL in `schema-init.ts` used a `PRIMARY KEY (COALESCE(...))` expression-column constraint, which SQLite rejects with `expressions prohibited in PRIMARY KEY and UNIQUE constraints`. That throw aborted segment 5 of `initLegacySchema` partway through on every launch with an existing DB, so `runMigrations(db)` was never reached — `user_version` stayed at 0 and migrations v1-v16 never applied, leaving the WC-4 INSERT statement pointed at a column that didn't exist. Fixed by moving the uniqueness rule from the `PRIMARY KEY` table constraint into a `CREATE UNIQUE INDEX` with the same expression columns (SQLite allows expressions in `UNIQUE INDEX`, just not in table constraints). The existing `ON CONFLICT(...)` upsert in `rag/store.ts` matches the new index unchanged. The regression test that should have caught this in `schema-init.test.ts` silently skipped under the better-sqlite3 NODE_MODULE_VERSION mismatch on every machine.
- ✅ **Wiring Closure Phase** (v0.9.1) — 11-prompt correctness phase that closes seven documented-but-dead-code gaps from FC + M + PRJ. `normalizeToolsForProvider` + `filterToolsForRole` now actually invoked from chat dispatch; implicit change contracts synthesized on first mutating tool call; migration v16 adds `messages.proof_status` so trust state is a structured column; `ProofGateBanner` + composer consume the persisted status; M9 composer appends deterministic `**Verification:**` footer citing each receipt; `verify:proof` is now invoked by CI as the M10 static gate; new `Sidebar.project-flow.test.ts` locks the original sidebar "+" defect with 13 source-reading wiring-contract assertions including the negative `window.prompt(` check. See [PLANNING/LAMPREY_WIRING_CLOSURE_PLAN.md](PLANNING/LAMPREY_WIRING_CLOSURE_PLAN.md).
