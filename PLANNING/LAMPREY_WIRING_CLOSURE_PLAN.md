...
| WC-2 | **Wire role-based tool access** | Apply `filterToolsForRole(descriptors, role)` before tools reach Planner / Reviewer / Coder. | `electron/services/agent-pipeline.ts`, `electron/services/role-tool-access.test.ts` additions | Unit: planner gets read-only subset; reviewer gets inspection subset; coder gets full set; integration: pipeline still completes a turn; lint; tsc node | [ ] |
| WC-3 | **Synthesize implicit contract on first mutation** | Before the first mutating tool call on a correlation id without an active contract, synthesize an `implicit: true` contract via `synthesizeImplicitChangeContract`. | `electron/ipc/chat.ts`, `electron/services/change-contract-store.ts`, tests | Unit: read-only turn does not synthesize; mutating turn without plan-mode contract synthesizes; implicit flag persists; lint; tsc node | [ ] |
| WC-4 | **Persist proof gate trust status** | Add `proof_status TEXT` column to `messages` (migration v16); chat write-through writes status when the gate evaluates. | `electron/services/schema-init.ts`, `electron/services/db-migrations.ts`, `electron/services/conversation-store.ts`, `electron/ipc/chat.ts`, `src/lib/types.ts`, tests | Unit: status persists for mutating turns, NULL for read-only; migration idempotent; lint; tsc node/web | [ ] |
| WC-5 | **UI + composer consume persisted trust status** | `ProofGateBanner.tsx` reads from `message.proofStatus` not from notice text; final response composer respects status; waiver flow updates status. | `src/components/chat/ProofGateBanner.tsx`, `electron/services/final-response-composer.ts`, `electron/services/proof-gate.ts`, `electron/ipc/contracts.ts` (waiver path), tests | Banner test asserts state-driven render; composer test asserts blocked/untrusted handling; waiver test flips status; lint; tsc web/node | [ ] |
| WC-6 | **Cite receipt IDs in final answer prose** | Inject inline receipt citations into the natural-language final answer when proof receipts exist (`receipt prf_abc123: vitest 142 passed, 0 failed`). | `electron/services/final-response-composer.ts`, tests | Composer test: prose contains receipt id when receipts present; absence stated clearly when missing; lint; tsc node | [ ] |
| WC-7 | **Wire verify:proof into CI** | Add `npm run verify:proof` step to `.github/workflows/ci.yml`; ensure script runs cleanly in CI environment. | `.github/workflows/ci.yml`, possibly `scripts/verify-proof.cjs` (only if env-specific adjustments are needed) | Local: script still passes; CI yaml lints; CI dry-run via `act` if available, otherwise documented; lint; tsc node | [ ] |
| WC-8 | **PRJ-10 end-to-end regression test** | Add a vitest component test that mounts `<Sidebar />`, clicks "+", asserts `<NewProjectModal />` renders, submits a name, asserts project IPC was called, asserts modal closes. | New `src/components/layout/Sidebar.project-flow.test.tsx`, mocks | New test passes and would have failed against the original `window.prompt()` "+"; lint; tsc web | [ ] |
| WC-9 | **Architecture doc accuracy sweep** | Update `ARCHITECTURE/FUNCTION_CALLING.md`, `ARCHITECTURE/MECHANICAL_PROOF.md`, `ARCHITECTURE/PROJECTS.md` to cite real invocation sites for each described invariant (now that WC-1 through WC-7 wired them). | The three architecture docs | Each invariant has a file:line citation that grep confirms; lint passes; no broken markdown links | [ ] |
| WC-10 | **Plan-record corrections** | Append a correction note to `PLANNING/Lamprey_Function_Calling_PSPR_.md` §3 FC-1B clarifying which tools were actually hardened (`shell_command, apply_patch, workspace_context, view_image`, not `read_file/write_file`); add a "Wiring Closure follow-up" subsection to each of the three audited plans pointing to this plan. | The three plan files | Notes added; existing prompt rosters not modified; lint passes | [ ] |
