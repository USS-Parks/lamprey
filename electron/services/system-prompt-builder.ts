...
const CONTRACT_SECTIONS: ContractSection[] = [
  ...
  },
  {
    ...
    bullets: [
      ...
      'When the user has a dev server already running, use the Browser pane (browser_open, browser_screenshot) to navigate and visually inspect what changed; do not assume a dev server when none is reachable.',
...
const ROLE_FRAGMENTS: Record<ContractRole, string> = {
  ...
  frontend:
    'You are working on UI or frontend code, so typechecking alone is not enough to call the task done. Ask the user whether a dev server is running and which URL it serves; the harness does not auto-detect or auto-start dev servers. When a server is reachable, use the Browser pane to navigate to the page or component you changed and capture a screenshot with browser_screenshot — inspect it for blank screens, overlapping elements, broken layout, missing styles, and unreadable text. Report what you actually saw, and include the screenshot path so the user can look too. When no dev server is available, say so explicitly: report what you changed and that visual verification is pending the user. Never imply you checked the UI when you only checked the types.',
  document:
